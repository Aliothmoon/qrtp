...
QTRP 是一个用 Go 编写的高性能反向代理和 TCP 隧道工具。它通过防火墙和 NAT 提供安全隧道，支持多种传输层和加密方法.
...
- **灵活的加密**
    - TLS 加密（自动生成自签名证书）
    - ChaCha20-Poly1305（64KB大帧，性能优于TLS）
    - 无加密（用于可信网络）
    - QUIC 传输内置 TLS 1.3，无需额外加密层
...
- **缓冲池**：512KB 缓冲区用于数据转发，减少 GC 压力
- **TCP 连接优化**：
    - 2MB 读写缓冲区（提升丢包网络的吞吐量）
    - TCP_NODELAY（禁用 Nagle 算法，降低延迟）
    - SO_KEEPALIVE（15秒间隔，快速检测死连接）
    - SO_REUSEADDR（支持快速重启）
...
- **QUIC + 任何加密方式**：QUIC 内置 TLS 1.3，会自动跳过额外的加密层（即使配置了 TLS 或 ChaCha20）
- **net/gnet + TLS**：标准 TLS 加密，自动生成自签名证书
- **net/gnet + ChaCha20**：高性能对称加密，需要配置预共享密钥（PSK）
- **net/gnet + none**：无加密，仅用于可信网络

### 性能调优关键点

1. **Smux 流缓冲区**：2MB 流缓冲区是实现高吞吐量的关键
    - 默认 256KB 会严重限制带宽
    - 在高延迟链路上尤为重要

2. **保活间隔**：15秒 ping 间隔针对弱网环境优化
    - 允许容忍 3 次丢包（45 秒总超时）
    - 比标准 30 秒间隔更激进
...
- 使用 HMAC-SHA256 防止令牌伪造
- 时间戳验证窗口：±5 分钟
- Nonce 缓存防止重放攻击（5分钟 TTL）
- 重连时自动清理旧会话
